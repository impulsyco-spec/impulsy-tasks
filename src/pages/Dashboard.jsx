...
export default function Dashboard() {
  ...
  if (loading) return (
    <div className="p-8 flex items-center gap-3 text-[rgb(var(--text-secondary))]">
      <div className="w-4 h-4 border-2 border-[rgb(var(--border))] border-t-[#12fcd9] rounded-full animate-spin" />
      Cargando...
    </div>
  )
  ...
  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-[14px] p-6">
          ...
          <div className="space-y-3">
            {profile?.organizations?.teams?.map(team => (
              <div key={team.id} className="flex items-center justify-between p-3 rounded-xl bg-[rgb(var(--card-hover))] border border-[rgb(var(--border))]">
                <span className="text-xs font-medium text-[rgb(var(--text-primary))]">{team.name}</span>
                <span className="text-[10px] text-[rgb(var(--text-secondary))]">{team.members?.[0]?.count || 0} miembros</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* Mis tareas */}
        <div className="lg:col-span-2 bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-[12px] overflow-hidden">
          <div className="flex items-center justify-between px-6 py-5 border-b border-[rgb(var(--border))]">
            <h3 className="text-[10px] font-bold uppercase tracking-wider text-[rgb(var(--text-muted))]">Mis tareas pendientes</h3>
            <button
          ...
          {myTasks.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-sm font-bold text-[rgb(var(--text-primary))]">¡Todo al día!</p>
              <p className="text-xs text-[rgb(var(--text-secondary))] mt-1">Has completado todas tus responsabilidades</p>
            </div>
          ) : (
              ...
              {myTasks.map(task => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center gap-4 px-6 py-4 hover:bg-[rgb(var(--card-hover))] cursor-pointer transition-all group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-[rgb(var(--text-primary))] group-hover:text-[#12fcd9] transition-colors truncate">{task.title}</p>
                      {task.due_date && (
                        <p className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${isOverdue ? 'text-[#ff4545]' : 'text-[rgb(var(--text-secondary))]'}`}>
        ...
        <div className="space-y-4">
          ...
          {isAdmin && (
            <button
              ...
              className="w-full flex items-center gap-4 p-5 bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-[12px] text-left hover:border-[rgba(18,252,217,0.5)] hover:bg-[rgb(var(--card-hover))] transition-all group"
            >
              ...
              <div className="relative z-10">
                <p className="text-sm font-bold text-[rgb(var(--text-primary))] uppercase tracking-wider">Subir transcript</p>
                <p className="text-[10px] text-[rgb(var(--text-secondary))] font-medium mt-0.5 uppercase tracking-widest">IA Task Extraction</p>
              </div>
              <ArrowRight size={16} className="text-[rgb(var(--text-muted))] ml-auto group-hover:text-[#12fcd9] group-hover:translate-x-1 transition-all" />
            </button>
          )}

          <button
            onClick={() => navigate('/tasks?filter=pending_approval')}
            className="w-full flex items-center gap-4 p-5 bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-[12px] text-left hover:border-[#f5a623] hover:bg-[rgb(var(--card-hover))] transition-all group"
          >
            ...
            <div>
              <p className="text-sm font-bold text-[rgb(var(--text-primary))] uppercase tracking-wider">Aprobar tareas</p>
              <p className="text-[10px] text-[rgb(var(--text-secondary))] font-bold uppercase tracking-widest mt-0.5">
                {stats.pending} pendiente{stats.pending !== 1 ? 's' : ''}
              </p>
            </div>
            <ArrowRight size={16} className="text-[rgb(var(--text-muted))] ml-auto group-hover:text-[#f5a623] group-hover:translate-x-1 transition-all" />
          </button>

          {isAdmin && (
            <button
              onClick={() => navigate('/teams')}
              className="w-full flex items-center gap-4 p-5 bg-[rgb(var(--card))] border border-[rgb(var(--border))] rounded-[12px] text-left hover:border-[#12fcd9] hover:bg-[rgb(var(--card-hover))] transition-all group"
            >
              <div className="w-10 h-10 rounded-[8px] bg-[rgba(18,252,217,0.1)] flex items-center justify-center flex-shrink-0 border border-[rgba(18,252,217,0.2)]">
                <Users size={18} className="text-[#12fcd9]" />
              </div>
              <div>
                <p className="text-sm font-bold text-[rgb(var(--text-primary))] uppercase tracking-wider">Equipos</p>
                <p className="text-[10px] text-[rgb(var(--text-secondary))] font-bold uppercase tracking-widest mt-0.5">Gestionar miembros</p>
              </div>
              <ArrowRight size={16} className="text-[rgb(var(--text-muted))] ml-auto group-hover:text-[#12fcd9] group-hover:translate-x-1 transition-all" />
            </button>
          )}
